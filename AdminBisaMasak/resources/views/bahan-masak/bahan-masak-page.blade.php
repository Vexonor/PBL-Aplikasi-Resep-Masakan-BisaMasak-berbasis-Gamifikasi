...
<div class="flex justify-between items-center gap-4 w-full">
    ...
    <!-- Add Button -->
    <a href="{{ route('bahan-masak.create') }}"
...
<div class="w-full min-h-screen bg-white p-5 my-5">
    ...
    <div class="w-full flex flex-col justify-center items-center gap-4 my-4">
        ...
        <h2 class="font-semibold text-xl">Saat ini, bahan masak masih belum tersedia di sistem.</h2>
    </div>
    @else
            ...
            <div class="p-1.5 min-w-full inline-block align-middle">
                <div class="shadow-xs overflow-hidden">
                    <table class="min-w-full divide-y divide-white">
                        ...
                        <tbody>
                            @foreach ($dataBahanMasak as $key => $bahanMasak )
                            <tr class="border border-cinnabar divide-x divide-cinnabar text-charcoal text-center">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <img class="inline-block size-9.5 rounded-full"
                                        src="{{ $bahanMasak -> gambar_bahan }}" alt="Avatar">
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    {{ $bahanMasak -> nama_bahan }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <p class="w-60 truncate mx-auto">
                                        {{ $bahanMasak -> deskripsi_bahan }}
                                    </p>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-end text-sm">
                                    <div class="flex justify-evenly items-center gap-4">
                                        <a href="{{ route('bahan-masak.edit', $bahanMasak -> id_bahan) }}">
                                        ...
                                        <a href="{{ route('bahan-masak.show', $bahanMasak -> id_bahan) }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="size-6" fill="currentColor"
                                                viewBox="0 0 256 256">
                                                <path
                                        ...
                                        <button type="button" class="cursor-pointer"
                                            data-modal-target="popup-modal{{ $bahanMasak -> id_bahan }}"
                                            data-modal-toggle="popup-modal{{ $bahanMasak -> id_bahan }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="size-6" fill="currentColor"
                                                viewBox="0 0 256 256">
                                                <path
    ...
    @endif
    <div class="my-5">
        {{ $dataBahanMasak -> links() }}
    </div>
</div>

@include('bahan-masak.delete-modal')
@endsection