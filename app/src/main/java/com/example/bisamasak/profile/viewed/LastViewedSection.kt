    ...
    Column (
        ...
        when(windowSize.widthSizeClass) {
            WindowWidthSizeClass.Compact -> {
                PortraitLastViewedSection()
            }
            WindowWidthSizeClass.Expanded -> {
                LandscapeLastViewedSection()
            }
        }
    }
}
...
@Composable
fun PortraitLastViewedSection() {
    val randomRecipes = rememberSaveable {
        DataProvider.ResepTerakhir.shuffled().take(2)
    }
    Row (
        modifier = Modifier
            .fillMaxWidth(),
...
@Composable
fun LandscapeLastViewedSection() {
    val randomRecipes = rememberSaveable {
        DataProvider.ResepTerakhir.shuffled().take(4)
    }
    Row (
        modifier = Modifier
            .fillMaxWidth(),
